...
__global__ void compute_tendencies_z_flux_kernel(double *d_state, double *d_flux, double dt) {
    ...
    int tile_height = blockDim.y + sten_size - 1;
    int tile_element_cnt = tile_width * tile_height;
    int tile_start_x = blockIdx.x * blockDim.x;
    int tile_start_y = blockIdx.y * blockDim.y;

    // Cooperatively load the output tile and the bottom-side stencil halo. The
    // state index intentionally starts in the top halo.
    if (i < d_nx) {
      for (ll = 0; ll < NUM_VARS; ll++) {
        for (int tile_y = threadIdx.y; tile_y < tile_height; tile_y += blockDim.y) {
          int tile_x = threadIdx.x;
          int state_x = tile_start_x + tile_x + hs;
          int state_y = tile_start_y + tile_y;
          if (state_y < d_nz + sten_size) {
            int smem_idx = ll * tile_element_cnt + tile_y * tile_width + tile_x;
            int state_idx = ll * d_state_var_stride
                          + state_y * d_state_row_stride
                          + state_x;
            smem[smem_idx] = d_state[state_idx];
          }
        }
    ...
    if (i < d_nx && k < d_nz + 1) {
        ...
        for (ll = 0; ll < NUM_VARS; ll++) {
            for (s = 0; s < sten_size; s++) {
                int smem_idx = ll * tile_element_cnt + (threadIdx.y + s) * tile_width + threadIdx.x;
                stencil[s] = smem[smem_idx];
            }
            // Fourth-order-accurate interpolation of the state
            vals[ll] =
...
__host__ void compute_tendencies_z_flux_host(double *d_state, double *d_flux, double dt) {
    ...
    int smem_width = block_dim.x;
    int smem_height = block_dim.y + sten_size - 1;
    int smem_size = NUM_VARS * smem_width * smem_height * sizeof(double);
...
__global__ void update_tendencies_z_kernel(double *d_state, double *d_flux, double *d_tend) {
  ...
  if (ll < NUM_VARS && k < d_nz && i < d_nx) {
    ...
    if (ll == ID_WMOM) {
      inds = ID_DENS * d_state_var_stride + (k + hs) * d_state_row_stride + i + hs;
